...
// Default poster columns from mock dramas
...
export default function LoginPage() {
  ...
  const [loginBanner, setLoginBanner] = useState<{ tagline:string; s1n:string; s1l:string; s2n:string; s2l:string; s3n:string; s3l:string; active:boolean; images?:string[] }>({ tagline:'Desbloqueia episódios com pontos. Sem mensalidades.', s1n:'500+', s1l:'Dramas', s2n:'2M+', s2l:'Fãs', s3n:'4.9★', s3l:'Avaliação', active:true, images:[] })
  ...
  const createUserDoc = async (uid: string, email: string, displayName: string, inviteCode?: string) => {
    ...
    if (!snap.exists()) {
      await setDoc(ref, {
        ...
        plan: 'Gratuito',
        coins: 40 + (inviteCode ? 20 : 0),  // +20 bônus para convidados
        status: 'Ativo',
        isAdmin: false,
      ...
      // Retroactively add referral code to existing users
  ...
  return (
    <div style={{
      ...
      {/* ── Left panel: animated poster wall ── */}
      <div className="login-left" style={{
        ...
        <div style={{
          ...
          background: [
            'linear-gradient(to bottom, #0a0a14 0%, transparent 18%, transparent 75%, #0a0a14 100%)',
            'linear-gradient(to right, #0a0a14 0%, transparent 20%)',
          ].join(', '),
        }} />

        {/* 3-column poster scroll */}
        <div style={{
          ...
          {(() => {
            ...
            // Split custom images into 3 columns (or use defaults)
              ...
              <div key={ci} style={{ flex:1, display:'flex', flexDirection:'column', gap:10, animation:col.anim }}>
                {col.items && col.items.length > 0
                  ...
                  : col.fallback.map((d, i) => (
                    <div key={i} style={{
                      borderRadius:14, aspectRatio:'2/3', flexShrink:0,
                      background: d.posterGrad ?? 'var(--rs-poster-romance)',
        ...
        <div style={{
          ...
          zIndex: 3, padding: '2rem',
          textAlign: 'center',
        }}>
          <Link href="/" style={{
            fontFamily: 'var(--rs-font-display)', fontWeight: 900, fontSize: '2.4rem',
            color: '#fff', textDecoration: 'none', display: 'block', marginBottom: '.5rem',
          }}>
            Reel<span style={{ color: 'var(--rs-primary)' }}>Story</span>
          </Link>
          <div style={{ fontSize: '.88rem', color: '#8892A4', marginBottom: '1.2rem' }}>
            {loginBanner.tagline}
          </div>
          {/* Stats */}
          <div style={{
            ...
            {[
              { num: loginBanner.s1n, label: loginBanner.s1l },
              { num: loginBanner.s2n, label: loginBanner.s2l },
              { num: loginBanner.s3n, label: loginBanner.s3l },
            ].map(s => (
      ...
      <div style={{
        ...
        <div style={{ width: '100%', maxWidth: 380 }}>
          ...
          <div className="login-mobile-logo" style={{
            ...
            <Link href="/" style={{
              fontFamily: 'var(--rs-font-display)', fontWeight: 900, fontSize: '2rem',
          ...
          {isAdminLogin && (
            <div style={{
              ...
              background: 'rgba(255,56,92,.12)', border: '1px solid rgba(255,56,92,.3)',
              borderRadius: 50, padding: '.3rem .9rem',
              fontSize: '.72rem', fontWeight: 800, color: 'var(--rs-primary)',
          ...
          {/* Title */}
          <h1 style={{
            fontFamily: 'var(--rs-font-display)', fontWeight: 900, fontSize: '1.8rem',
            margin: '0 0 .3rem',
          }}>
            {mode === 'login' ? (isAdminLogin ? 'Admin ReelStory' : 'Bem-vindo de volta') : 'Criar conta'}
          </h1>
          <p style={{
            fontSize: '.88rem', color: '#8892A4',
            margin: '0 0 2rem',
          }}>
            {mode === 'login'
              ? (isAdminLogin ? 'Entra com a conta de administrador' : 'Entra na tua conta ReelStory')
              : 'Junta-te a ReelStory hoje'}
          </p>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '.85rem' }}>
            ...
            {error && (
              <div style={{
                ...
                borderRadius: 10,
                padding: '.65rem .9rem',
                fontSize: 'var(--rs-body-sm)', color: '#ef4444',
              }}>
                {error}
              </div>
            )}
          ...
          <div style={{
            ...
            fontSize: 'var(--rs-body-sm)', color: '#8892A4',
          }}>
            {mode === 'login' ? (
              <>
                ...
                <button
                  ...
                  style={{
                    color: 'var(--rs-primary)', fontWeight: 700,
